<p align="center">
  <img src="https://raw.githubusercontent.com/chelslava/chelslava/main/banner.png" width="100%" />
</p>

<br/>
<p align="center">
  <b>🇷🇺 <a href="#russian">Русский</a></b> • 
  <b>🇺🇸 <a href="#english">English</a></b>
</p>

---

<a id="russian"></a>

## 👋 Вячеслав Челищев

**Senior RPA Developer · Tech Lead · AI Engineer**

Разрабатываю и проектирую системы автоматизации уровня enterprise:
от RPA-процессов до AI-ассистентов и архитектуры интеграций.

---

### 🧠 Экспертиза

* RPA архитектура (Dispatcher / Performer, очереди, retry, failover)
* AI системы (LLM, RAG, классификация)
* Автоматизация бизнес-процессов и интеграции
* Высоконагруженная обработка данных (Excel, API, scraping)

---

### ⚙️ Технологии

![C#](https://img.shields.io/badge/C%23-.NET-512BD4?style=flat-square\&logo=dotnet)
![Python](https://img.shields.io/badge/Python-AI-3776AB?style=flat-square\&logo=python)
![RPA](https://img.shields.io/badge/RPA-Automation-00C853?style=flat-square)
![AI](https://img.shields.io/badge/AI-LLM%20%7C%20RAG-FF6F00?style=flat-square)

---

### 🚀 Проекты

**🧩 [RPAForge](https://github.com/chelslava/rpaforge)**
Фреймворк автоматизации с упором на масштабируемую архитектуру и переиспользование компонентов.

**🤖 CoachIT**
AI-ассистент для анализа резюме, вакансий и генерации сопроводительных писем.

**⚙️ REFramework++**
Расширенный RPA-framework с поддержкой Dispatcher / Performer и устойчивой обработки ошибок.

---

### 📊 GitHub

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=chelslava&show_icons=true&hide_border=true&bg_color=00000000" height="140"/>
  <img src="https://streak-stats.demolab.com/?user=chelslava&theme=dark&hide_border=true" height="140"/>
</p>

---

### 🐍 Активность

<picture>
  <source
    media="(prefers-color-scheme: dark)"
    srcset="https://chelslava.github.io/chelslava/github-snake-dark.svg"
  />
  <source
    media="(prefers-color-scheme: light)"
    srcset="https://chelslava.github.io/chelslava/github-snake.svg"
  />
  <img
    alt="GitHub contribution snake"
    src="https://chelslava.github.io/chelslava/github-snake-dark.svg"
  />
</picture>

---

### 📫 Контакты

Telegram: https://t.me/ChelSlava
Email: [chelslava@gmail.com](mailto:chelslava@gmail.com)

---

<a id="english"></a>

## 👋 Vyacheslav Chelischev

**Senior RPA Developer · Tech Lead · AI Engineer**

Designing and building enterprise-grade automation systems:
from RPA workflows to AI assistants and scalable architectures.

---

### 🧠 Expertise

* RPA architecture (Dispatcher / Performer, queues, retry, failover)
* AI systems (LLM, RAG, classification)
* Business process automation & integrations
* High-load data processing (Excel, API, scraping)

---

### ⚙️ Tech Stack

![C#](https://img.shields.io/badge/C%23-.NET-512BD4?style=flat-square\&logo=dotnet)
![Python](https://img.shields.io/badge/Python-AI-3776AB?style=flat-square\&logo=python)
![RPA](https://img.shields.io/badge/RPA-Automation-00C853?style=flat-square)
![AI](https://img.shields.io/badge/AI-LLM%20%7C%20RAG-FF6F00?style=flat-square)

---

### 🚀 Projects

**🧩 [RPAForge](https://github.com/chelslava/rpaforge)**
Automation framework focused on scalable architecture and reusable components.

**🤖 CoachIT**
AI assistant for resume analysis, job matching, and cover letter generation.

**⚙️ REFramework++**
Advanced RPA framework with Dispatcher / Performer execution model.

---

### 📊 GitHub

![Stats](https://github-readme-stats.vercel.app/api?username=chelslava\&show_icons=true\&theme=default)
![Streak](https://streak-stats.demolab.com/?user=chelslava\&theme=default)

---

### 🐍 Activity

<picture>
  <source
    media="(prefers-color-scheme: dark)"
    srcset="https://chelslava.github.io/chelslava/github-snake-dark.svg"
  />
  <source
    media="(prefers-color-scheme: light)"
    srcset="https://chelslava.github.io/chelslava/github-snake.svg"
  />
  <img
    alt="GitHub contribution snake"
    src="https://chelslava.github.io/chelslava/github-snake-dark.svg"
  />
</picture>

---

### 📫 Contact

Telegram: https://t.me/ChelSlava
Email: [chelslava@gmail.com](mailto:chelslava@gmail.com)

---

### ⚡

```text
Automation | AI | RPA
Building systems, not scripts
```
